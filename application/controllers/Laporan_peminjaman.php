	...
	public function index(){
	  ...
	  $excel->getProperties()->setCreator('My Notes Code')
	   ...
	   'borders' => array(
	    ...
	    $excel->setActiveSheetIndex(0)->setCellValue('E3', "Nama Petugas"); // Set kolom E3 dengan tulisan "nama petugas"
	    ...
	    foreach($peminjaman as $data){ // Lakukan looping pada variabel data      
		    ...
		    $excel->setActiveSheetIndex(0)->setCellValue('E'.$numrow, $data->nama_petugas);  
		  ...
		  $excel->getActiveSheet(0)->setTitle("Laporan Data Peminjaman");    $excel->setActiveSheetIndex(0);    
		  // Proses file excel    
		  header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');    header('Content-Disposition: attachment; filename="Data Peminjaman.xlsx"'); 
	  ...
	  public function pdf(){
        ...
        $pdf->Cell(40,6,'Nama Petugas',1,1);
        $pdf->SetFont('Arial','',10);
        $i = 1;
        $peminjaman = $this->peminjaman->getAllPeminjaman();
        foreach ($peminjaman as $row){
        	 	$pdf->Cell(8,6,$i,1,0);
            $pdf->Cell(40,6,$row->nama_anggota,1,0);
            $pdf->Cell(40,6,$row->judul,1,0);
            $pdf->Cell(40,6,$row->tgl_pinjam,1,0);
            $pdf->Cell(40,6,$row->nama_petugas,1,1); 
            $i++;
        }
        $pdf->Output();
	  }
